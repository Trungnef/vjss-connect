  ...
  return (
    <article className="relative overflow-hidden">
      <section className="page-hero border-b border-border/70 border-beam">
        <div
          className={cn(
            "site-shell grid gap-10 py-16 sm:py-20 lg:py-24",
            hasSidebar && "lg:grid-cols-[minmax(0,1fr)_21rem] xl:grid-cols-[minmax(0,1fr)_23rem]",
          )}
        >
          <div className="max-w-4xl">
            <div className="flex flex-wrap items-center gap-2 text-[11px] uppercase tracking-[0.16em] text-foreground/48">
            ...
            <h1 className="mt-6 max-w-5xl font-serif text-4xl font-semibold leading-[1.04] text-balance sm:text-5xl lg:text-[4.35rem]">
              {title}
            </h1>
            {description ? (
              <p className="mt-6 max-w-3xl text-base leading-8 text-foreground/76 sm:text-lg">
                {description}
              </p>
            ) : null}
            {actions ? <div className="mt-8 flex flex-wrap gap-3">{actions}</div> : null}
            {meta?.length ? (
              <dl className="hero-meta-grid mt-10">
                {meta.map((item, index) => (
            ...
            {(heroNote || meta?.length) && <div className="editorial-rule mt-8 max-w-sm" />}
            {heroNote ? (
              <p className="mt-5 max-w-2xl text-sm leading-6 text-foreground/62">{heroNote}</p>
            ) : null}
          </div>

          {hasSidebar ? (
            <aside className="self-start space-y-5 lg:sticky lg:top-28">
              {aside ? <div className="panel-card panel-card-strong p-5">{aside}</div> : null}
              {quickLinks?.length ? (
